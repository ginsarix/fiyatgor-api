...
export async function registerAuthRoutes(app) {
    app.post("/auth/sessions", zValidator("json", authFormValidation), async (c) => {
        ...
        if (!user) {
            ...
            await bcryptCompare("dumdum", "$2a$12$nz3oVrDh0JBmUbGgzDdGUec/FrN7LBU8wDXab.Jc.KJKltOk93.u."); // just a dummy hash
        ...
        const sessionId = await setSession(user.id, firm.serverCode, user.role, user.name);
        // if the return value of setSession is null the most likely situation is a conflict because of the NX option
        if (!sessionId)
            return c.json({
                message: "Oturumunuz oluşturulamadı, zaten giriş yapılı olabilirsiniz",
            }, 403);
