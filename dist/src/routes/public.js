...
export function registerPublicRoutes(app) {
    app.get("/servers/:serverCode/products/:barcode", zValidator("param", z.object({
        ...
        if (!firmId) {
            return c.json({ message: `${serverCode} sunucu kodlu bir firma bulunamadı` }, 404);
        }
        const product = await findProductByAnyBarcode(db, firmId, barcode);
        if (!product)
            return c.json({ message: "Ürün bulunamadı" }, 404);
        return c.json({
            product,
